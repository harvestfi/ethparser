...
public class IncomeDBService {
  ...
  public boolean saveIncome(UniswapDTO dto) {
    ...
    List<Double> amountSumL = uniswapRepository
        .fetchAmountSum(dto.getBlockDate(), DO_HARD_WORK, limitOne);
    if (amountSumL != null && !amountSumL.isEmpty()) {
      amountSum = amountSumL.get(0);
    }

    double amountSumUsd = 0.0;
    List<Double> amountSumUsdL = uniswapRepository
        .fetchAmountSumUsd(dto.getBlockDate(), DO_HARD_WORK, limitOne);
